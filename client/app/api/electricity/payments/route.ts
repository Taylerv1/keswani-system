...
export async function GET(req: NextRequest) {
  try {
    ...
    const endpoint = `${API_BASE}/bill-payments${query ? `?${query}` : ""}`;
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Failed to fetch electricity payments",
      },
      { status: 500 }
    );
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const backendRes = await fetch(`${API_BASE}/bill-payments`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Failed to create electricity payment",
      },
      { status: 500 }
    );
  }
